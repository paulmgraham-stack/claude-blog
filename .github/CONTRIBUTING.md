...
- Treat any new untrusted-data path (file from user, network response, agent output, project-root file) as crossing a trust boundary. Fence content explicitly (see the `blog-researcher` role guidance for the WebFetch pattern; see `skills/blog/SKILL.md` "Untrusted-Data Contract" section for the project-root pattern).
